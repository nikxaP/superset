...
| `list_databases` | List database connections with filtering and search |
| `get_database_info` | Get database metadata (name, backend type, permissions) |
| `list_datasets` | List datasets with filtering and search |
| `get_dataset_info` | Get dataset metadata (columns, metrics, filters) |
| `create_virtual_dataset` | Create a virtual (SQL-based) dataset |
...
| `get_chart_type_schema` | Get the configuration schema for a specific chart type |
| `generate_chart` | Create a new chart (preview mode by default; pass `save_chart=true` to persist) |
...
| `save_sql_query` | Save a SQL query to SQL Lab's Saved Queries list |
